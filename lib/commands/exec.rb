  ...
  module Commands
    module Exec
      ...
      def run_execute(path_arg, target_dir, input: nil, debug: false)
        ...
        config_path = File.join(target_dir, ::CONFIG_FILENAME)

        unless File.exist?(config_path)
          warn "Error: No #{::CONFIG_FILENAME} found in #{target_dir}."
          warn "       Run '#{::CLI_NAME} init' first."
          exit 1
        end
        ...
        if connector_path.nil? || connector_file.nil?
          warn "Error: #{::CONFIG_FILENAME} is missing connector path/file. Run '#{::CLI_NAME} init' again."
          exit 1
        end
        ...
        pairs.each do |section, key|
          ...
          unless File.directory?(fixtures_dir)
            ...
            warn "       Run '#{::CLI_NAME} exec #{section}.#{key} --mode=fields' first."
            next
          end
          ...
          if input_files.empty?
            ...
            # warn "         Run '#{::CLI_NAME} exec #{section}.#{key} --mode=fields' to generate templates,"
            # warn '         then remove the sentinel comment from execute_input.json.'
            next
          end
